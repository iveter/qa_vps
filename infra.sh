...
echo "Everything you will do with your Sandbox needs to be done under user we are going to create!"
...
cp -r ./sandbox /home/${NEW_USER} |& tee -a ${LOG_FILE_NAME}

chown -R ${NEW_USER}:users /home/${NEW_USER}/sandbox |& tee -a ${LOG_FILE_NAME}
...
runuser -l ${NEW_USER} -c 'cd ~/sandbox && docker-compose up -d'
...
JENKINS_PASSWORD=$(docker exec -t sandbox_jenkins_1 cat /var/jenkins_home/secrets/initialAdminPassword)
...
echo "Sandbox files are here: /home/$NEW_USER/sandbox" |& tee -a ${LOG_FILE_NAME}
